# Claude Instructions for HonoCom

## Project Overview

HonoCom is a modular e-commerce platform built with TypeScript, implementing Domain-Driven Design (DDD) architecture with clean separation of concerns across domain, application, and infrastructure layers.

## Architecture

- **Monorepo structure** using pnpm workspaces and Turbo
- **Domain-Driven Design** with clear boundaries between contexts
...
honocom/
├── apps/
│   └── api/           # Hono web API application
├── packages/
│   ├── domain/        # Domain entities, value objects, events
│   │   ├── catalog/   # Product domain context
│   │   ├── orders/    # Order domain context
│   │   └── shared-kernel/  # Shared domain primitives
│   ├── application/   # Use cases, handlers, repositories
│   │   ├── catalog/   # Product application services
│   │   └── orders/    # Order application services
│   └── infrastructure/  # Data access, external services
│       ├── catalog/   # Product persistence
│       ├── orders/    # Order persistence
│       └── shared/    # Database connections, etc.
```

## Development Commands

- **Development**: `pnpm dev` (runs all services in watch mode)
- **Build**: `pnpm build` (builds all packages and apps)
- **Test**: `pnpm test` (runs all test suites)
- **Lint**: `pnpm lint` (lints all packages)
- **Type Check**: `pnpm check-types` (TypeScript compilation check)
...
export class Product extends AggregateRoot<ProductId> {
  static create(/* params */): Product {
    // Factory method with validation and event emission
  }
...
- **Repository Implementations**: Concrete data access using SQL
- **Database Connections**: Centralized connection management
...
- `@domain/catalog` - Domain catalog package
- `@domain/shared-kernel` - Shared domain primitives
- `@application/catalog` - Application catalog services
- `@infrastructure/shared` - Infrastructure utilities

### Testing Patterns

- **Unit Tests**: Test domain logic and business rules
- **Integration Tests**: Test application handlers with repositories
- **Naming**: `*.test.ts` files co-located with source code
- **Configuration**: Jest with ts-jest for TypeScript support

### Error Handling

- Domain exceptions for business rule violations
- Application layer validates commands and coordinates
- Infrastructure layer handles technical failures

## Database Strategy

- SQL-based persistence with prepared statements
- Repository pattern abstracts database specifics
- Entity-to-row mapping in infrastructure layer
...
4. **API Integration**: Wire everything together in Hono routes
5. **Testing**: Write comprehensive tests at each layer

### Code Quality

- Follow existing TypeScript configuration
- Use strict null checks and proper typing
- Implement proper error handling and validation
- Write meaningful tests covering business scenarios
- Follow DDD patterns consistently across contexts
...
1. Create entity class extending `AggregateRoot`
2. Define value objects for entity properties
3. Implement business methods with invariants
4. Add domain events for significant changes
5. Create repository interface in application layer
6. Implement concrete repository in infrastructure

### Adding a New API Endpoint

1. Define command/query structures
2. Implement handler in application layer
3. Create repository methods if needed
4. Add Hono route in API application
5. Wire dependencies with proper error handling

### Database Schema Changes

1. Update repository implementations
2. Modify entity mapping logic
3. Consider migration strategy for existing data
4. Update tests to reflect schema changes

Always run `pnpm check-types` and `pnpm test` before committing changes.
